#!/usr/bin/env python3
"""
🤖 VPS BOT HOSTING MANAGER - Telegram Bot v2.0 (Advanced) - FIXED
BOT MADE BY @Hx5x5x5x
Fixes applied: Made by @Dev_Null_X
  - get_logs(): pm2's own [TAILING]/meta banner no longer leaks into log output
  - HostedBot.start() (python branch): now returns the REAL start status instead
    of always returning True, so "Stopped" bots show the actual failure reason
  - Removed a stray systemctl call for pm2-managed python bots (no unit exists)
"""
import os
import sys
import asyncio
...
try:
    ...
    ADMIN_IDS = DEFAULT_CONFIG["ADMIN_IDS"]
    HOSTED_BOTS_DIR = DEFAULT_CONFIG["HOSTED_BOTS_DIR"]
    LOGS_DIR = DEFAULT_CONFIG["LOGS_DIR"]
    MAX_BOTS = DEFAULT_CONFIG["MAX_BOTS"]
    MAX_ZIP_SIZE_MB = DEFAULT_CONFIG["MAX_ZIP_SIZE_MB"]
    AUTO_START_AFTER_DEPLOY = DEFAULT_CONFIG["AUTO_START_AFTER_DEPLOY"]
...
def safe_execute(func):
    async def wrapper(update: Update, context: ContextTypes.DEFAULT_TYPE, *args, **kwargs):
        try:
            ...
            log_error(f"Error in {func.__name__}: {str(e)}", extra={"user_id": update.effective_user.id})
    return wrapper
...
class HostedBot:
    ...
    def get_logs(self, lines=30):
        """
        FIX: added --raw so pm2 doesn't inject its own colored meta banner
        (the "[1m [90m[TAILING] Tailing last N lines..." junk), and strip any
        leftover pm2 meta lines as a safety net so only the bot's real
        stdout/stderr shows up.
        """
        try:
            result = subprocess.run(
                ["pm2", "logs", self.service_name, "--lines", str(lines), "--nostream", "--raw"],
                capture_output=True, text=True, timeout=15
            )
            raw = result.stdout or ""
            cleaned = strip_ansi(raw)
            # drop pm2's own banner/meta lines, keep only actual app output
            filtered_lines = [
                line for line in cleaned.splitlines()
                if "[TAILING]" not in line and not line.strip().startswith("[PM2]")
            ]
    ...
    def start(self):
        try:
            if self.bot_type == "nodejs":
                ...
                # ---- PYTHON BOTS ----
                # FIX: this branch used to always `return True` at the end,
                # regardless of whether pm2 actually managed to start the
                # process. That's why the panel showed "Stopped" with no
                # explanation - the failure was being swallowed. It now
                # returns the real outcome, and we also stop calling
                # `systemctl start <service>` here since these bots are
                # pm2-managed only; no systemd unit is ever created for them,
                # so that call was a silent no-op that could mask errors.
                ...
                # pm2 can report a successful launch that then immediately
                # crashes (bad import, missing dep, syntax error). Give it a
                # moment and check the real process status before reporting
                # success back to the user.
                time.sleep(1)
                if not self.is_running():
                    log_error(
                        f"PM2 reported start for {self.name} but process is not online "
                        f"(likely crashed on launch - check `pm2 logs {self.service_name}`)"
                    )
                    return False
                return True
        except Exception as e:
    ...
    def install_npm_package(self, package_name):
        try:
            result = subprocess.run(
                ...
                cwd=self.bot_dir, capture_output=True, text=True, timeout=3000
            )
...
async def handle_action(update: Update, context: ContextTypes.DEFAULT_TYPE, bot_id: str, action: str):
    ...
    try:
        status = f"{S.SUCCESS} {action_texts[action]}!" if success else f"{S.ERROR} Failed! Check logs for details."
...
async def delete_bot(update: Update, context: ContextTypes.DEFAULT_TYPE, bot_id: str):
    ...
    try:
        await query.edit_message_text(
            f"{S.LOADING} Deleting **{info['name']}**...",
            parse_mode="Markdown"
        )
    except:
        pass
    ...
    await safe_edit(query,
        f"{S.SUCCESS} **Deleted!**\n\n"
        f"{S.BOT} {info['name']} has been removed.",
        InlineKeyboardMarkup([
...
async def show_pkg_manager_menu(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    for bot_id, info in bots.items():
        type_emoji = {"nodejs": S.NODEJS, "python": S.PYTHON, "whatsapp": S.WHATSAPP}.get(info['type'], S.BOT)
        keyboard.append([InlineKeyboardButton(
            f"{type_emoji} {info['name']} ({info['type']})",
...
async def show_installed_packages(update: Update, context: ContextTypes.DEFAULT_TYPE, bot_id: str):
    ...
    text = f"{S.PACKAGE} **Packages: {info['name']}**\n{'─' * 25}\n\n"
...
async def message_handler(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if context.user_data.get('awaiting_zip') and update.message.document:
        await handle_zip_deploy(update, context)
        return

    if context.user_data.get('awaiting_github_url') and update.message.text:
        await handle_github_deploy(update, context)
        return

    if context.user_data.get('awaiting_single_file') and update.message.document:
        await handle_single_file_deploy(update, context)
        return

    if context.user_data.get('awaiting_premium_add') and update.message.text:
        try:
    ...
    if context.user_data.get('awaiting_premium_remove') and update.message.text:
        try:
            target_id = int(update.message.text.strip())
            if PremiumManager.remove_premium(target_id):
                await update.message.reply_text(
    ...
    if context.user_data.get('awaiting_wa_password') and update.message.text:
    ...
    if context.user_data.get('awaiting_wa_phone') and update.message.text:
    ...
    if context.user_data.get('awaiting_npm_pkg') and update.message.text:
    ...
    if context.user_data.get('awaiting_pip_pkg') and update.message.text:
        ...
        if bot_type == "whatsapp":
            ...
            if AUTO_START_AFTER_DEPLOY:
                ...
                if bot_obj.start():
                    ...
                    start_text = f"\n{S.WARNING} Auto-start failed. Check View Logs for the reason."
...
async def handle_single_file_deploy(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if not (document.file_name.endswith('.js') or document.file_name.endswith('.py')):
