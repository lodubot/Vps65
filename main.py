...
GitHub deploy, single file deploy, auto dependency install
...
# ==================== CONFIG DEFAULTS ====================
DEFAULT_CONFIG = {
    "BOT_TOKEN": os.getenv("BOT_TOKEN", "8663644709:AAFbnAHJTWo_j3Dok1FZ_UniULoEA8vqbmY"),
    "ADMIN_IDS": [int(x) for x in os.getenv("ADMIN_IDS", "8761475920").split(",") if x.strip()],
...
async def safe_edit(query, text, reply_markup=None):
    # If the current message is media (e.g. the /start video), edit_message_text
    # will fail since you can't convert a media message into a plain text message.
    # In that case, delete it and send a fresh text message instead.
    ...
    try:
        ...
        if "Message is not modified" in str(e):
            ...
            # Fallback: message turned out to be non-text after all — send fresh
            try:
                await query.message.delete()
            except:
                pass
...
async def confirm_delete(update: Update, context: ContextTypes.DEFAULT_TYPE, bot_id: str):
    ...
    info = bots[bot_id]  # <-- FIXED: dedented from inside the 'if' block
    await safe_edit(query,
